...
int coststrategy = MAXIMIZE_COST; // default
...
static u8* describe_op(u8 hnb, u8 new_score) {
  ...
  //RK: add suffix for highscore, YN: and also for lowscore
  if (new_score) {
    if (coststrategy == MAXIMIZE_COST) {
      strcat(ret, ",highscore");
    } else if (coststrategy == MINIMIZE_COST) {
      strcat(ret, ",lowscore");
    }
  }
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  // RK: true if this is a new highscore, YN: changed new_highscore -> new_score
  u8 new_score = 0;
  ...
  if (fault == crash_mode) {
    ...
    if (coststrategy == MAXIMIZE_COST) {
      new_score = more_costly_than_highscore();
    } else if (coststrategy == MINIMIZE_COST) {
      new_score = less_costly_than_lowscore();
    } else {
      new_score = 0;
    }
  ...
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      fn = alloc_printf("%s/hangs/id:%06llu,%s", out_dir,
                        unique_hangs, describe_op(0, new_score));
...
keep_as_crash:
      ...
      fn = alloc_printf("%s/crashes/id:%06llu,sig:%02u,%s", out_dir,
                        unique_crashes, kill_signal, describe_op(0, new_score));
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  /* Fields in the file:
     ...
     execs_per_sec, lowscore, highscore */

  fprintf(plot_file,
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %llu, %llu\n",
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          unique_hangs, max_depth, eps, lowscore, highscore); /* ignore errors */
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, lowscore, highscore\n");
                     /* ignore errors */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qc:s:")) > 0)
    ...
    switch (opt) {
      ...
      /* YN: option to specifiy cost strategy: maximization or minimization. */
      case 's':
        if (optarg && !strcmp(optarg, "max"))
          coststrategy = MAXIMIZE_COST;
        else if (optarg && !strcmp(optarg, "min"))
          coststrategy = MINIMIZE_COST;
        else if (optarg)
          FATAL("Unknown cost optimization strategy %s", optarg);
        else
          FATAL("Cost optimization strategy expected after -s flag");
  ...
  // YN: print which cost optimization strategy we are Using
  if (coststrategy == MAXIMIZE_COST)
    OKF("[WCA] Try to maximize cost");
  else if (coststrategy == MINIMIZE_COST)
    OKF("[WCA] Try to minimize cost");
  else
    WARNF("Not using any cost optimization strategy!");
